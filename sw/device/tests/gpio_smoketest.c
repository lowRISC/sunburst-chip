 ...
 * `hw/top_chip/dv/env/seq_lib/chip_sw_gpio_smoke_vseq.sv`. The DV test
...
bool test_main(void) {
  ...
  for (size_t i = 0; i < kDifGpioNumPins; ++i) {
    if (gpio_mask & (1u << i)) {
      ...
      dif_pinmux_index_t gpio_out = kTopChipPinmuxOutselGpioGpio0 + i;
      CHECK_DIF_OK(dif_pinmux_output_select(&pinmux, mio, gpio_out));
      mio = kPinmuxTestutilsGpioInselPins[i];
      dif_pinmux_index_t gpio_in = kTopChipPinmuxPeripheralInGpioGpio0 + i;
