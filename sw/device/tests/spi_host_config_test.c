...
bool test_main(void) {
  ...
  base_addr = mmio_region_from_addr(TOP_CHIP_SPI_HOST1_BASE_ADDR);
...
static status_t configure_pinmux(const dif_pinmux_t *pinmux) {
  // CSB.
  TRY(dif_pinmux_output_select(pinmux, kTopChipPinmuxMioOutIor10,
                               kTopChipPinmuxOutselSpiHost1Csb));
  // SCLK.
  TRY(dif_pinmux_output_select(pinmux, kTopChipPinmuxMioOutIor11,
                               kTopChipPinmuxOutselSpiHost1Sck));
  // SD0.
  TRY(dif_pinmux_input_select(pinmux, kTopChipPinmuxPeripheralInSpiHost1Sd0,
                              kTopChipPinmuxInselIor12));
  TRY(dif_pinmux_output_select(pinmux, kTopChipPinmuxMioOutIor12,
                               kTopChipPinmuxOutselSpiHost1Sd0));

  // SD1.
  TRY(dif_pinmux_input_select(pinmux, kTopChipPinmuxPeripheralInSpiHost1Sd1,
                              kTopChipPinmuxInselIor13));
  TRY(dif_pinmux_output_select(pinmux, kTopChipPinmuxMioOutIor13,
