...
void ottf_external_isr(uint32_t *exc_info) {
  ...
  switch (peripheral) {
    case kTopChipPlicPeripheralUart0:
      if (!ottf_console_flow_control_isr(exc_info)) {
        goto unexpected_irq;
      };
      break;
    case kTopChipPlicPeripheralI2c0: {
      // Check that the ID matches the expected interrupt, then mask it, since
      // it's a status type.
      dif_i2c_irq_t irq =
          (dif_i2c_irq_t)(plic_irq_id - kTopChipPlicIrqIdI2c0FmtThreshold);
      i2c_isr(irq);
    } break;
    default:
      goto unexpected_irq;
...
static status_t test_init(void) {
  ...
  base_addr = mmio_region_from_addr(TOP_CHIP_I2C0_BASE_ADDR);
  ...
  rv_plic_testutils_irq_range_enable(&plic, kPlicTarget,
                                     kTopChipPlicIrqIdI2c0FmtThreshold,
