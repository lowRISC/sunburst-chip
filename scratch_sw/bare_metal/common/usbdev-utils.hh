...
class UsbdevUtils {
 public:
  UsbdevUtils(CHERI::Capability<volatile OpenTitanUsbdev> &dev, const uint8_t *device,
              uint8_t device_len,                    // Device Descriptor.
              const uint8_t *cfg, uint16_t cfg_len,  // Configuration Descriptor.
              const uint8_t *test, uint8_t test_len)
      :  // Test Descriptor.
        usbdev(dev),
        devAddr(0u),
  ...
  bool connected() { return usbdev->connected(); }
  ...
  typedef void (*UsbdevRecvCB)(void *handle, uint8_t ep, bool setup, const uint8_t *data, uint16_t pktLen);
 ...
 private:
  ...
  static void ep0_recv_cb(void *handle, uint8_t ep, bool setup, const uint8_t *pktData, uint16_t pktLen) {
 ...
 private:
  ...
  // Device state
  enum DeviceState { Device_Powered, Device_Addressed, Device_Configured } devState;

  // Control Transfer state
  enum CtrlState { Ctrl_Setup, Ctrl_StatusSetAddr, Ctrl_StatusGetDesc, Ctrl_StatusSetConfig } ctrlState;
