...
//      {OpenTitanI2cInterrupt::ControllerHalt, false},
  ...
  void pattgen_test() {
    ...
    struct pattgen_irq {
      SunburstPattgen::SunburstPattgenInterrupt id;
      bool can_clear;
    };
    static constexpr std::array<pattgen_irq, 2> pattgenMap = {{
        {SunburstPattgen::SunburstPattgenInterrupt::InterruptDoneCh0, true},
        {SunburstPattgen::SunburstPattgenInterrupt::InterruptDoneCh1, true},
    }};

    auto pattgen = pattgen_ptr(root);

    // Ensure that initially all interrupts are cleared and disabled.
    pattgen->interrupt_disable(static_cast<SunburstPattgen::SunburstPattgenInterrupt>(~0u));
    pattgen->interruptState = ~0u;
    pattgen->interruptTest  = 0u;

    log_->println("  Testing Pattgen:", instance);
    for (size_t i = 0; i < pattgenMap.size(); ++i) {
      ip_irq_id        = pattgenMap[i].id;
      is_irq_clearable = pattgenMap[i].can_clear;
      plic_irq_id      = static_cast<PLIC::Interrupts>(PLIC::Interrupts::Pattgen + instance);
      exp_intr_state   = pattgenMap[i].id;

      // Register interrupt handler.
      irq_handler = [](PlicTest *plic_test, PLIC::Interrupts irq) {
        auto pattgen           = pattgen_ptr(plic_test->root);
        const bool wrong_irq   = (irq != plic_test->plic_irq_id);
        const bool wrong_local = !(pattgen->interruptState & plic_test->ip_irq_id);
        plic_test->error_count += wrong_irq;
        plic_test->error_count += wrong_local;
        plic_test->log(irq, pattgen->interruptEnable & pattgen->interruptState, wrong_irq || wrong_local);
        plic_test->pattgen_irq_clear(pattgen);
      };
      // Enable and trigger the interrupt now that the handler has been registered.
      pattgen->interrupt_enable(pattgenMap[i].id);
      pattgen->interruptTest = ip_irq_id;
      if (!irq_caught()) {
        pattgen_irq_clear(pattgen);
      }
    }
  }

  void pattgen_irq_clear(PattgenPtr pattgen) {
    if (is_irq_clearable) {
      pattgen->interruptState = ip_irq_id;
    } else {
      // Ensure that the `intr_test` bit does not keep the Status-type interrupt asserted.
      pattgen->interruptTest = 0;
    }
    // Disable interrupt to prevent interference with other tests.
    pattgen->interrupt_disable(static_cast<SunburstPattgen::SunburstPattgenInterrupt>(ip_irq_id));
  }
  ...
  void spi_test(size_t spi_instance) {
    ...
    struct spi_irq {
      SunburstSpi::SunburstSpiInterrupt id;
      bool can_clear;
    };
    static constexpr std::array<spi_irq, 2> spiMap = {{
        {SunburstSpi::SunburstSpiInterrupt::InterruptError, true},

        {SunburstSpi::SunburstSpiInterrupt::InterruptSpiEvent, false},
    }};

    auto spi = spi_ptr(root, instance);

    // Ensure that initially all interrupts are cleared and disabled.
    spi->interrupt_disable(static_cast<SunburstSpi::SunburstSpiInterrupt>(~0u));
  ...
  void spi_irq_clear(SpiPtr spi) {
    ...
    // Disable interrupt to prevent interference with other tests.
    spi->interrupt_disable(static_cast<SunburstSpi::SunburstSpiInterrupt>(ip_irq_id));
  }
